<?php
	...
	class IbridoJsonController extends DMJsonController {
		...
		**/
		...
		function jsonProcess() {
			...
			if ($stockTarget == 1) {
				...
				if (!$isPackage) {
					$currentSession = DMSession::get('ibridoChargelists', false);
					if (!$currentSession) {
					...
					DMSession::set('ibridoChargelists', $currentSession);
				...
				//	}
					...
					//Se è un cartone ibrido, devo gestirne la tracciabilità!
		...
		function jsonAggregatePackage() {
			...
			FHHelper::log("fh", "[ibrido.jsonAggregatePackage] Starting aggregation for $packages packages of article $articleId");
			...
			$myPrintClassPath = DM_APP_PATH . DS . 'views' . DS . 'ibrido' . DS . 'print.php';
			...
			$printResult = $myPrintClass->execPrint('ibrido', 'default');
