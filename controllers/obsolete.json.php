<?php
	...
	class ObsoleteJsonController extends DMJsonController {
		...
		**/
		...
		function jsonAggregatePackage() {
			...
			FHHelper::log("fh", "[obsolete.jsonAggregatePackage] Starting aggregation for $packages packages of article $articleId");
			...
			$currentSession = DMSession::get('obsoleteChargelists', false);
			$currentSession[$chargelistId][$articleId] -= ($myArticle->package_units * $packages);
			
			$data = new StdClass();
			
			//Ora devo generare la stampa delle etichette
			$myPrintClassPath = DM_APP_PATH . DS . 'views' . DS . 'obsolete' . DS . 'print.php';
			...
			$printResult = $myPrintClass->execPrint('obsolete', 'default');
