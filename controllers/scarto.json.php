<?php
	...
	class ScartoJsonController extends DMJsonController {
		...
		**/
		...
		function jsonProcess() {
			...
			if ($stockTarget == 1) {
				...
				if (!$isPackage) {
					$currentSession = DMSession::get('scartoChargelists', false);
					if (!$currentSession) {
					...
					DMSession::set('scartoChargelists', $currentSession);
				...
				//	}
					...
					//Se è un cartone scarto, devo gestirne la tracciabilità!
		...
		function jsonAggregatePackage() {
			...
			FHHelper::log("fh", "[scarto.jsonAggregatePackage] Starting aggregation for $packages packages of article $articleId");
			...
			$myPrintClassPath = DM_APP_PATH . DS . 'views' . DS . 'scarto' . DS . 'print.php';
			...
			$printResult = $myPrintClass->execPrint('scarto', 'default');
