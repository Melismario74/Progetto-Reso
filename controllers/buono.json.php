<?php
	...
	class BuonoJsonController extends DMJsonController {
		...
		**/
		...
		function jsonProcess() {
			...
			if ($stockTarget == 1) {
				...
				if (!$isPackage) {
					$currentSession = DMSession::get('buonoChargelists', false);
					if (!$currentSession) {
					...
					DMSession::set('buonoChargelists', $currentSession);
		...
		function jsonAggregatePackage() {
			...
			FHHelper::log("fh", "[buono.jsonAggregatePackage] Starting aggregation for $packages packages of article $articleId");
			...
			$myPrintClassPath = DM_APP_PATH . DS . 'views' . DS . 'buono' . DS . 'print.php';
			...
			$printResult = $myPrintClass->execPrint('buono', 'default');
