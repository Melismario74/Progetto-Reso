	...
	class FHUdmHelper {
		...
		**/
		...
		/**
			...
			Mario ok
		**/
		function getudms($searchParams, &$totalResults){
		...
		/**
			Ottiene lo stato di tutte le udm con quantità diversa da 0
			modificata da Mario per view UDM
		**/
		function getUdmsMod($searchParams, &$totalResults){
			...
			if ((isset($searchParams['udmCode'])) && ($searchParams['udmCode'] != '')) {
				$whereConditions .= " AND u.udm_code = '" . DMDatabase::escape($searchParams['udmCode']) . "'";
			}
			if ((isset($searchParams['ubicazione'])) && ($searchParams['ubicazione'] != '')) {
				$whereConditions .= " AND u.ubicazione = '" . DMDatabase::escape($searchParams['ubicazione']) . "'";
			}
		...
		**/
		...
		static function exportUdmsCSV($params) {
			...
			$totalResults = 0;
			$udms = self::getudms($params, $totalResults);
		...
		static function exportUdmsCSVmod($params) {
			...
			$totalResults = 0;
			$udms = self::getUdmsMod($params, $totalResults);
